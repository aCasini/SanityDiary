...
def get_ai_narrative_analysis(df):
    ...
    prompt = f"Paziente: Alessio Casini. Post-Embolia Polmonare. Analizza: {summary}"
    try:
...
def export_pdf(df, ai_comment):
    ...
    # FIX TypeError: output() ora richiede il parametro 'dest' o restituisce i byte direttamente a seconda della versione
    return pdf.output(dest='S').encode('latin-1')

# --- 5. LOGICA APPLICATIVA ---
if supabase:
    # Sidebar: spostata FUORI dal controllo 'if not df.empty' per essere sempre visibile
    with st.sidebar:
    ...
    if not df.empty:
        ...
        st.info("Benvenuto! Inizia inserendo la tua prima misurazione nel pannello a sinistra ⬅️")
